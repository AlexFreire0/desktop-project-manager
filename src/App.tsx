...
const appInitials = 'PD';

type View = 'overview' | 'projects' | 'commands' | 'settings';
type Notice = { kind: 'success' | 'error' | 'info'; text: string };
type ProjectDialogState = { mode: 'new'; path?: string } | { mode: 'edit'; project: Project } | null;
...
function App() {
  ...
  const [gitStatus, setGitStatus] = useState<Record<string, GitInfo>>({});
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <nav>
          <NavItem icon="⌂" label="Overview" active={view === 'overview'} onClick={() => { setSelectedId(undefined); setView('overview'); }} />
          <NavItem icon="▦" label="All projects" count={data.projects.length} active={view === 'projects' && !selected} onClick={() => { setSelectedId(undefined); setView('projects'); }} />
          <NavItem icon="✦" label="Favorites" count={data.projects.filter((project) => project.favorite).length} active={view === 'projects' && !selected && filter === 'favorites'} onClick={() => { setSelectedId(undefined); setView('projects'); setFilter('favorites'); }} />
      ...
      <main className="main-content">
        <header className="topbar"><div className="breadcrumb"><span>Workspace</span><b>/</b><strong>{selected && view === 'projects' ? selected.name : view === 'overview' ? 'Overview' : view === 'projects' ? 'Projects' : view === 'commands' ? 'Quick commands' : 'Settings'}</strong></div><div className="top-actions"><button className="icon-button" title="Open project folder" onClick={() => window.desktop.chooseFolder().then((folder) => folder && setProjectDialog({ mode: 'new', path: folder }))}>＋</button><div className="avatar" aria-label="Project Desk">{appInitials}</div></div></header>
        {notice && <div className={`notice ${notice.kind}`}><span>{notice.kind === 'success' ? '✓' : notice.kind === 'error' ? '!' : 'i'}</span>{notice.text}</div>}
        {view === 'overview' && <Overview projects={data.projects} recent={recent} commands={data.quickCommands} gitStatus={gitStatus} onNavigate={setView} onSelect={(id) => { setSelectedId(id); setView('projects'); }} onRun={executeCommand} onAdd={() => setProjectDialog({ mode: 'new' })} />}
        {view === 'projects' && !selected && <ProjectsView projects={visibleProjects} allCount={data.projects.length} query={query} setQuery={setQuery} filter={filter} setFilter={setFilter} sort={sort} setSort={setSort} compact={data.settings.compactCards} selectedId={selectedId} gitStatus={gitStatus} onSelect={setSelectedId} onAdd={() => setProjectDialog({ mode: 'new' })} onEdit={(project) => setProjectDialog({ mode: 'edit', project })} onDelete={deleteProject} onToggleFavorite={(project) => saveProject({ ...project, favorite: !project.favorite, updatedAt: new Date().toISOString() })} />}
        {view === 'projects' && selected && <ProjectDrawer project={selected} settings={data.settings} onClose={() => setSelectedId(undefined)} onEdit={() => setProjectDialog({ mode: 'edit', project: selected })} onSave={saveProject} onNotice={showNotice} />}
      ...
      {projectDialog && <ProjectDialog initial={projectDialog.mode === 'edit' ? projectDialog.project : undefined} initialPath={projectDialog.mode === 'new' ? projectDialog.path : undefined} onClose={() => setProjectDialog(null)} onSave={saveProject} />}
...
function Overview({ projects, recent, commands, gitStatus, onNavigate, onSelect, onRun, onAdd }: { projects: Project[]; recent: Project[]; commands: QuickCommand[]; gitStatus: Record<string, GitInfo>; onNavigate: (view: View) => void; onSelect: (id: string) => void; onRun: (command: QuickCommand) => void; onAdd: () => void }) {
  const now = new Date();
  const dateLabel = new Intl.DateTimeFormat(undefined, { weekday: 'long', month: 'long', day: 'numeric' }).format(now);
  const greeting = now.getHours() < 12 ? 'Good morning' : now.getHours() < 18 ? 'Good afternoon' : 'Good evening';
  return <div className="page"><div className="hero"><div><p className="eyebrow">{dateLabel}</p><h1>{greeting} <span>✦</span></h1><p className="muted">Your workspace, organized around the way you build.</p></div><button className="primary-button" onClick={onAdd}>＋ Add project</button></div>
    ...
    {recent.length ? <div className="project-grid">{recent.map((project) => <ProjectCard key={project.id} project={project} git={gitStatus[project.path]} onClick={() => onSelect(project.id)} />)}</div> : <EmptyState title="Your workspace is ready" text="Add your first project to keep folders, git status, and commands together." action="Add your first project" onAction={onAdd} />}
...
function ProjectDialog({ initial, initialPath, onClose, onSave }: { initial?: Project; initialPath?: string; onClose: () => void; onSave: (project: Project) => void }) {
  const [form, setForm] = useState<Project>(() => initial || { id: uid(), name: '', path: initialPath || '', description: '', color: colors[Math.floor(Math.random() * colors.length)], favorite: false, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), parts: [] });
